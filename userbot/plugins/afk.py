...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await event.edit(f"My Boss is going!away from keyboard, and Reason is {reason}")
        else:
            await event.edit(f"My Boss is Going. Bye")
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        message_to_reply = f"My Master Has Been Gone For {afk_since}\nWhere He Is: **Only God Knows** " + \
            f"\n\n__ I'll back in a few Billion Light years__\n**REASON**: {reason}" \
            if reason \
