...
async def _(event):
    ...
    if is_a_s:
        ...
        if userid == 813878981:
            packname = f"Real KIng ka pack"
            packshortname = "realkingisgreat"
        else:
            packname = f"{user.username}'s {pack}"
            packshortname = f"Natural_{userid}" # format: Uni_Borg_userid
